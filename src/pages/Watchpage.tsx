...
export default function Watchpage() {
  ...
  useEffect(() => {
    const checkStatus = async () => {
      if (videoId && user) {
        try {
          const res = await api.get<{ isSaved: boolean; id?: string }>(
            `library/status/${videoId}`,
          );
  ...
  const lastSyncedProgressRef = useRef<{ videoId: string; time: number }>({
    videoId: "",
    time: 0,
  });
  ...
  useEffect(() => {
    const generateTopics = async () => {
      if (
        ...
        videoId &&
        videoTranscript &&
        !videoDataMap[videoId]?.topics &&
        !isGeneratingTopics
      ) {
        setIsGeneratingTopics(true);
        try {
          const res = await api.post<{ data: Topic[] }>("gemini/topics", {
            videoId,
            videoTranscript,
          });
          setVideoDataMap((prev) => ({
    ...
    const init = async () => {
      if (videoId) {
        ...
        if (!queue.find((item) => item.id === videoId)) {
          ...
          setQueue((prev) =>
            prev.find((i) => i.id === videoId)
              ? prev
              : [...prev, { id: videoId, title }],
          );
        }
  ...
  useEffect(() => {
    console.log("[Transcript Info]: ", videoTranscript)
  }, [videoTranscript]);
  ...
  const handleVideoEnded = useCallback(async () => {
    ...
    if (!user || !videoId || !videoTranscript || videoTranscript.length < 100) {
      console.log(
        "[Watchpage] Missing requirements for final quiz, skipping...",
      );
      handleNextVideo();
      return;
    }
      ...
      if (user) {
        try {
          ...
          setVideoDataMap((prev) => ({
            ...
            [videoId]: {
              ...prev[videoId],
              notes: prev[videoId].notes.map((n) =>
                n.id === id ? res.data : n,
              ),
            },
          }));
  ...
  const handleSendMessage = useCallback(
    async (text: string) => {
      ...
      if (!user) {
        setNotification(
          "AI features are available for logged-in students only!",
        );
